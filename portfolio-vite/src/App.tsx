...
const DESKTOP_GLOBE_WIDTH = 80;
const DESKTOP_GLOBE_HEIGHT = 70;
const MOBILE_GLOBE_WIDTH = 60;
const MOBILE_GLOBE_HEIGHT = 45;
...
function useAsciiGlobe(isMobile: boolean, globeSize: { width: number; height: number }, starFieldSize: { width: number; height: number }) {
  ...
  React.useEffect(() => {
    function render() {
      let output = Array(starFieldSize.height)
        .fill('')
        .map(() => Array(starFieldSize.width).fill(' '));
      // Draw stars
      for (let s of stars.current) {
        let sx = Math.floor(s.x);
        let sy = Math.floor(s.y);
        if (sy >= 0 && sy < starFieldSize.height && sx >= 0 && sx < starFieldSize.width) {
          let tw = Math.abs(Math.sin(s.twinkle));
          output[sy][sx] = tw > 0.5 ? s.char : ' ';
        }
      }
      // Center the globe in the starfield
      const cx = Math.floor(starFieldSize.width / 2) - 5;
      ...
      for (let y = 0; y < globeHeight; y++) {
        for (let x = 0; x < globeWidth; x++) {
          let dx = (x - globeWidth / 2) / radius;
          let dy = (y - globeHeight / 2) / radius * 1.6;
          let dist = dx * dx + dy * dy;
          if (dist <= 1.0) {
            let dz = Math.sqrt(1.0 - dist);
            let xp = dx;
            let yp = dy;
            let zp = dz;
            let angle = angleRef.current;
            let xr = xp * Math.cos(angle) - zp * Math.sin(angle);
            let zr = xp * Math.sin(angle) + zp * Math.cos(angle);
            let u = Math.atan2(xr, zr) / (2 * Math.PI) + 0.5;
            let v = (yp + 1) / 2;
            let tx = Math.floor(u * texW);
            let ty = Math.floor(v * texH);
            if (ty >= 0 && ty < texH && tx >= 0 && tx < texW) {
              let ch = texture[ty][tx];
              const px = Math.floor(cx - globeWidth / 2 + x);
              const py = Math.floor(cy - globeHeight / 2 + y);
              if (py >= 0 && py < starFieldSize.height && px >= 0 && px < starFieldSize.width) {
                output[py][px] = ch === '.' ? ' ' : ch;
              }
            }
          }
...
function PlusCrossIcon({ open, onClick }: { open: boolean; onClick: () => void }) {
    ...
    >
      ...
      >
        <svg width="28" height="28" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M10.1153 1.96182H11.8259L11.9393 8.37204H10.2287L10.1153 1.96182ZM10.2287 12.6574H11.9393L11.8259 19.0676H10.1153L10.2287 12.6574ZM13.2267 9.65941H19.5235V11.37L13.2267 11.37V9.65941ZM2.41772 9.65941H8.94134V11.37H2.41772V9.65941Z" fill="white"/>
        </svg>
      </span>
    </button>
  );
...
export default function App() {
  ...
  const [starFieldSize, setStarFieldSize] = useState(() => (
    isMobile
      ? {
          width: MOBILE_GLOBE_WIDTH + (MOBILE_GLOBE_WIDTH % 2 === 0 ? 5 : 4),
          height: MOBILE_GLOBE_HEIGHT + (MOBILE_GLOBE_HEIGHT % 2 === 0 ? 5 : 4),
        }
      : {
  ...
  const handleMenuClose = () => {
    setMenuOpen(false);
    setTimeout(() => setIconCross(false), 200);
  };

  return (
    <div style={{ minHeight: '100vh', minWidth: '100vw', background: '#000', fontFamily: 'Ubuntu Mono, monospace', display: 'flex', flexDirection: 'column', width: '100vw', height: '100vh' }}>
      {/* Top bar */}
      <div
        style={{
          width: '100vw',
      ...
      >
        <div style={{ fontWeight: 400, fontSize: isMobile ? 18 : 20, letterSpacing: 1, marginBottom: 6 }}>
          likhochan.
        </div>
        {isMobile ? (
          ...
          <div style={{ display: 'flex', gap: 40, alignItems: 'center', marginBottom: 6 }}>
            {menuItems.slice(1).map((item) => (
              <a key={item.label} href={item.href} style={{ color: '#fff', textDecoration: 'none', fontSize: 20, fontFamily: 'Ubuntu Mono, monospace', fontWeight: 400 }}>{item.label}</a>
            ))}
          </div>
        )}
      </div>
      ...
      {menuOpen && isMobile && (
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', padding: '0 18px', height: MENU_TOPBAR_HEIGHT, fontSize: 18 }}>
            <div style={{ fontWeight: 400, marginBottom: 6, letterSpacing: 1 }}>likhochan.</div>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 6 }}>
              <PlusCrossIcon open={true} onClick={handleMenuClose} />
            </div>
          </div>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-start', paddingLeft: 24, gap: 24 }}>
            {menuItems.map((item) => (
              <div
                key={item.label}
                style={{
              ...
              >
                <span style={{ width: 18, display: 'inline-block', textAlign: 'left', fontWeight: item.current ? 700 : 400 }}>
                  {item.current ? '>' : ''}
                </span>
      ...
      <div style={{ 
        ...
        justifyContent: 'center', 
        width: '100vw', 
        height: `calc(100vh - ${(isMobile ? MENU_TOPBAR_HEIGHT : TOPBAR_HEIGHT)}px)`, 
        background: '#000',
        overflow: 'hidden',
        padding: 0,
        margin: 0
